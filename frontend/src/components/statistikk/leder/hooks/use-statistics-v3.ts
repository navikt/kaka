...
import { useStatisticsManager } from '@app/simple-api-state/statistics/v3/use-statistics-manager';
...
export const useManagerStatisticsV3IsLoading = (): boolean => useStatistics().isLoading;

const EMPTY_ARRAY: IFullStatisticVurderingV3[] = [];
const EMPTY_SAKSBEHANDLERE: Record<string, IFullStatisticVurderingV3[]> = {};

export const useFilteredManagerStatisticsV3 = () => {
  ...
  const filter = useCallback(
    ({ ytelseId, sakstypeId, utfallId, hjemmelIdList, tilbakekreving }: IFullStatisticVurderingV3) =>
  ...
  return useMemo(
    () => ({
      ...
      saksbehandlere: Object.entries(saksbehandlere).reduce<Record<string, IFullStatisticVurderingV3[]>>(

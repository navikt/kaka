...
import { copyTable } from '@app/components/echarts/copy-table';
import { downloadChartDataAsCsv } from '@app/components/echarts/csv-download';
import { DataViewTable } from '@app/components/echarts/data-view-table';
import { copyChartAsPng } from '@app/components/echarts/png-clipboard';
import { downloadChartAsPng } from '@app/components/echarts/png-download';
...
import { init, registerLocale, registerTheme, use } from 'echarts/core';
...
registerTheme(AppTheme.DARK, DARK_THEME);
registerTheme(AppTheme.LIGHT, LIGHT_THEME);
registerLocale('nb-NO', nbNO);
...
export const EChart = ({
  ...
  useEffect(() => {
    ...
    eChartsRef.current = init(ref.current, theme, { locale: 'nb-NO' });
  ...
  return (
    <VStack height={chartHeight === undefined ? '100%' : undefined} width="100%" gap="space-16">
      <VStack align="center" position="relative">
        <HStack gap="space-8" paddingInline="space-24" justify="center">
          <Heading
            ...
            style={{ height: headingHeight ?? 'auto' }}
          >
            <span>
              {title}

              <span className="ml-2 inline-flex gap-2">
                {extraTitleContent === undefined ? null : <span>{extraTitleContent}</span>}

...
const getTableHtml = (table: HTMLTableElement): string => {
  const headerCells = Array.from(table.querySelectorAll<HTMLTableCellElement>('thead>tr>th'), toCell).join('\n');

  const rows = Array.from(table.querySelectorAll<HTMLTableRowElement>('tbody>tr'), (row) => {
    const cells = Array.from(row.childNodes).filter(isCellElement).map(toCell);
